...
-----------------------------------------fin de la configuración original---------------------------------------------
...
30/01/2020 Ya se han comentado todas las líneas de control de la cámara de vídeo y del LCD
Le añado dos DHT22 y así estarán los 3 que me hacen falta, uno para la temperatura interior del casco, y dos para cada uno de los tubos de protección de los motores, el del casco lo quitaré en su momento, puesto que con el giroscopio GY MPU6050, he conseguido medir también la temperatura, por lo tanto tendré dos sensores que me indica la temperatura interior del ROV.
He borrado varias líneas de control de la cámara que no se va a utilizar

01/02/2020 Le añado los controles del giroscopio MPU6050, facilitados por Dea de www.cambatronics.com

02/02/2020 Le añado la orden para leer solo una vez X segundo el DHT22, truco de José Luis de
...
Se modifica la estructura del código siguiendo esas órdenes de programación, según la información obtenida y guardada por mi en la carpeta 01_UNIR_2_CODIGOS.
Se crean varios bloques en el void loop, para el funcionamiento de cada uno de los sensores, eliminando todos los delay().
...
Serial.begin(57600);     // Puerto Serie USB, para comunicaciones con el ordenador

Serial1.begin(57600);    // Puerto Serie en los pines 18 y 19, para comunicaciones con el otro Arduino.

ETin.begin(details(rxdata), &Serial1);    // Comienza la recepción de datos a través de Serial, en los pines 18 y 19, con el otro Arduino.

ETout.begin(details(txdata), &Serial1);   // Comienza el envío de datos a través de Serial, en los pines 18 y 19, con el otro Arduino.

28/03/2020 Se incluyen el envió de todas las instrucciones del mando, para tener todos los botones operativos (no funciona bien aún)
...
También se incluye el segundo sensor de presión SKU237545

05/04/2020 Se cambia el código del sensor infrarrojo E18D80NK, por uno mejor.

11/04/2020 Corrijo el código según las instrucciones de mi amigo Atilano Belinchón, y el código mejora bastante.
...
Al pulsar R1, el servo inclina el eje donde están situados los tubos de los motores, y el ROV debe subir a superficie, al pulsar R2, el eje se inclina en sentido contrario y el ROV debe bajar ganando profundidad.

18/04/2020 Se cambian algunos " if ", por " else if ", para darle agilidad al código, siguiendo las instrucciones de DEA de
...
05/05/2020 Se incluye la Macro F, en todos los Serial.print, para reducir tanto el espacio ocupado por el programa, y el consumo de memoria dinámica, según los consejos de Bigbadbob de  https://www.homebuiltrov.com , pero me da problemas.
...
13/05/2020 Se modifica el control de los motores incluyendo la librería ESC.h, del archivo RC_ESC_master, a partir de aquí, en vez de controlar los ESC como un servo, se controla como lo que son ESC.

16/05/2020 Se hacen diversos cambios, según los consejos de Tsanshon de www.foromodelismonaval.es para agilizar el código.
...
08/06/2020 después de muchas pruebas y modificaciones, consigo que los programas compilen en VSC + PIO, pero no puedo conseguir hacer mucho mas, puesto que no hay librería especifica para el mando de PS2.

09/06/2020 Consigo introducir librerías especificas con la ayuda de Maxgerhardt de Platforio fórum, según las publicaciones del fórum
https://community.platformio.org/t/un-rov-de-agua-dulce-a-sweet-water-rov/14157

11/06/2020 Me registro en Github, abro una publicación y subo los códigos, que siguen sin funcionar correctamente.
