...
#include <ESC.h> // Libreria para el control de los ESC de los motores
...
struct RECEIVE_DATA_STRUCTURE  // estructura de mensaje de datos recibidos de los sensores
{
  ...
  int ROVDepth;             // Recibe mensaje de datos de lectura de profundidad de ROV (m)
  ...
  int V5;                   // Recibe mensaje de datos de voltaje de batería de 5V al maestro.
  int V12;                  // Recibe mensaje de datos de voltaje de batería de 12V al maestro.

};

struct SEND_DATA_STRUCTURE  // estructura de mensaje de datos enviados
{
  int upLraw;  //Variables para transportar los datos reales sin procesar de los ESCs
  int upRraw;
  int HLraw;
  int HRraw;
  int CamPitch;  // Angulo del servo de la camara
  ...
  volatile boolean LEDHdlts;            // Manda que hacer con los LEDs, encendidos/apagados
  volatile boolean LEDHdlts1;           // Manda que hacer con los Focos, encendidos/apagados
};
...
void setup()
{
  ...
  txdata.CamPitch = 90; // Configura el tono de la cámara para que esté nivelado
  txdata.upLraw = 1500; // Establecer motores verticales de potencia neutra inicial
  txdata.upRraw = 1500; // Establecer motores izquierdo de potencia neutra inicial
  txdata.HLraw = 1500; // Establecer motores derecho de potencia neutra inicial
  txdata.HRraw = 1500; // Establecer motores derecho de potencia neutra inicial
