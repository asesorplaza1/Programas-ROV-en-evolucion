...
const int led1 = 40;  // El pin del LED indicador de batería de 5V es el 40.
const int led2 = 42;  // El pin del LED indicador de batería de 12V es el 42.
const int led3 = 44;  // El Pin utilizado como chivato de recibir datos del mando
...
const int led6 = 50;  // El pin chivato del mando es el 5
...
const int Refrigeracion = 52; // El pin utilizado para encender la bomba de refrigeración es el 34
...
#define CURRENT_SENSOR A0   // pin AO de Arduino, donde va la salida del sensor 5V
#define CURRENT_SENSOR A1   // pin AO de Arduino, donde va la salida del sensor 12V
float amplitude_current;    // amplitud corriente 5V
float effective_value;      // corriente efectiva 5V
float amplitude_current1;   // amplitud corriente 12V
float effective_value1;     // corriente efectiva 12V
...
struct RECEIVE_DATA_STRUCTURE
{
  ...
  volatile boolean LEDHdlts1;            // Luces LED encendidas / apagadas
};
...
{
  int value;                  // Manda mensaje de datos de voltaje de batería de 5V al maestro.
  int value1;                 // Manda mensaje de datos de voltaje de batería de 12V al maestro.
...
void loop ()
{
  ...
  for (int i = 0; i < 5; i++) {// Esta función, cambia el estado de envio a recepción cada 5 segundos
    ...
    ETout.sendData();       // Manda el mensaje al puerto serie para el Arduino del ROV
    {
      ...
      loop_vIN();             // Bloque de trabajo para medir el voltaje de la batería de 5V
      loop_vIN1();            // Bloque de trabajo para medir el voltaje de la batería de 12V
      loop_average;           // Bloque de tranajo del Amperimetro de la bateria de 5V
      loop_average1;          // Bloque de tranajo del Amperimetro de la bateria de 12V
...
void loop_vIN() {// Bloque de trabajo de medición de voltaje de la batería 5V
...
void loop_vIN1() {// Bloque de trabajo de medición de voltaje de la batería 12V
...
void loop_average() // Bloque de trabajo del Amperimetro de 5V
{
  float average = 0;
...
void loop_average1()  // Bloque de trabajo del Amperimetro de 12V
{
  float average1 = 0;
  for (int i = 0; i < 1000; i++)
  {
